"""GET /api/infra/batteries — viewport query (PostGIS EIA 860M)."""
...
from app.db.models.infrastructure import BatteryAsset
...
def batteries(
    ...
    filters = [
        ST_Intersects(BatteryAsset.geom, envelope),
        BatteryAsset.capacity_mw >= min_capacity_mw,
        BatteryAsset.status == status,
    ]
    if iso:
        filters.append(BatteryAsset.iso.in_(iso))

    stmt = (
        select(
            BatteryAsset.id,
            BatteryAsset.eia_plant_id,
            BatteryAsset.eia_generator_id,
            BatteryAsset.name,
            BatteryAsset.owner,
            BatteryAsset.iso,
            BatteryAsset.state,
            BatteryAsset.capacity_mw,
            BatteryAsset.capacity_mwh,
            BatteryAsset.duration_hours,
            BatteryAsset.status,
            BatteryAsset.cod_date,
            BatteryAsset.retirement_date,
            func.ST_Y(BatteryAsset.geom).label("lat"),
            func.ST_X(BatteryAsset.geom).label("lon"),
        )
        .where(and_(*filters))
        .limit(limit + 1)
    )
    ...
    for r in rows:
        ...
        ret_dt = r.retirement_date.isoformat() if r.retirement_date else None
        data.append(
            {
                "id": r.id,
                ...
                "capacityMwh": float(r.capacity_mwh) if r.capacity_mwh is not None else None,
                "durationHours": float(r.duration_hours) if r.duration_hours is not None else None,
                "status": r.status,
                "codDate": cod,
                "retirementDate": ret_dt,
            }
        )
    ...
    return build_envelope(
        ...
        data=data,
        summary=f"{len(data):,} battery assets in viewport ({iso_label}, {status}).",
    )
