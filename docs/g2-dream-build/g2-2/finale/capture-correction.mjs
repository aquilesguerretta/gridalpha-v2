...
const directory=path.join(path.dirname(fileURLToPath(import.meta.url)),'correction-02');
...
for(const width of [1440,390]) for(const mode of ['light','dark']) {
 ...
 try{
  ...
  const finalGeometry=await s.evaluate(`(()=>{const r=q=>{const a=document.querySelector(q).getBoundingClientRect();return{top:a.top,bottom:a.bottom,width:a.width,height:a.height}};return{width:innerWidth,overflow:document.querySelector('.g2-shell').scrollWidth>innerWidth,imageLoaded:document.querySelector('.g22-finale-figure img').naturalWidth>0,figure:r('.g22-finale-figure'),question:r('.g22-finale-open-question'),action:r('.g22-finale-enter')}})()`);
