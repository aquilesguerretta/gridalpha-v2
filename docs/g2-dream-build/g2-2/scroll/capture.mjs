...
const root='C:/dev/gridalpha-v2-nivar-g2/docs/g2-dream-build/g2-2/scroll';
...
for(const width of [1440]){
 ...
 await ev('document.fonts.ready');await delay(1200);await ev('document.querySelector(".g2-shell").scrollTo({top:0,behavior:"instant"})');
 const bounds={x:0,y:0,width,height:1000};
 ...
 await delay(3000);
 for(let i=0;i<29;i++){
  await call('Input.dispatchMouseEvent',{type:'mouseWheel',x:1090,y:720,deltaX:0,deltaY:470});await delay(850);
  const state=await ev('({url:location.href,width:innerWidth,scrollWidth:document.documentElement.scrollWidth,scrollTop:document.querySelector(".g2-shell").scrollTop,scrollHeight:document.querySelector(".g2-shell").scrollHeight,images:[...document.querySelectorAll("main img")].filter(e=>{const r=e.getBoundingClientRect();return r.bottom>0&&r.top<innerHeight}).map(e=>({src:e.currentSrc,loaded:e.complete&&e.naturalWidth>0}))})');observations.push(state);
  if(i%4===0||i===28){const shot=await call('Page.captureScreenshot',{format:'png'});await fs.writeFile(path.join(out,`portal-${i}.png`),Buffer.from(shot.data,'base64'));}
 }
 await delay(1700);
 ...
 const result=spawnSync('ffmpeg',['-hide_banner','-loglevel','error','-f','concat','-safe','0','-i',path.join(temp,'frames.txt'),'-vf',`crop=${fw}:${fh}:${Math.floor(bounds.x)}:${Math.floor(bounds.y)},fps=30`,'-an','-c:v','libx264','-crf','21','-pix_fmt','yuv420p','-movflags','+faststart','-y',path.join(out,'portal-scroll-desktop.mp4')],{encoding:'utf8'});if(result.status)throw Error(result.stderr);
