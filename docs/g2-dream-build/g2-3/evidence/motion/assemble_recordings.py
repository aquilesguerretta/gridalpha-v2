...
def assemble(name, source, frames, crop=None):
    assert len(frames) > 1
    ...
    decoder = subprocess.Popen([FFMPEG, '-hide_banner', '-loglevel', 'error', '-i', str(master), '-fps_mode', 'passthrough', '-f', 'rawvideo', '-pix_fmt', 'rgb24', 'pipe:1'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    for path in input_paths:
        pixels = decoder.stdout.read(width * height * 3)
        assert pixels == Image.open(path).convert('RGB').tobytes(), f'RGB master mismatch: {path.name}'
    assert not decoder.stdout.read(1), 'Unexpected decoded frame after all source frames'
    decoder.wait()
    assert decoder.returncode == 0, decoder.stderr.read().decode('utf-8', errors='replace')
...
if __name__ == '__main__':
    ...
    if args.name:
        ...
        results = [assemble(args.name, source, frames, crop)]
    else:
